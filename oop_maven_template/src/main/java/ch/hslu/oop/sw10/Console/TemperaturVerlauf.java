...
public final class TemperaturVerlauf {
	...
	public void add(Temperatur temp) {
		if (temp != null && (temp instanceof Temperatur)) {
			store.add(temp);
			// every time a Temperatur-Objekt is added, it it checked if the newly added
			// Object is a new max odr min Object. If it is, then a TemperaturEvent is fired
			if (this.getCount() >= 2) {
				if (currentMin != this.getMin().getKelvin()) {
					TemperaturEvent event = new TemperaturEvent(this, MaxOrMinEnum.MIN, this.getMin().getKelvin());
					this.fireTemperaturEvent(event);
					currentMin = this.getMin().getKelvin();
				}

				if (currentMax != this.getMax().getKelvin()) {
					TemperaturEvent event = new TemperaturEvent(this, MaxOrMinEnum.MAX, this.getMax().getKelvin());
					this.fireTemperaturEvent(event);
					currentMin = this.getMin().getKelvin();
				}
			}
	...
	public String toString() {
		...
		if (this.getCount() == 0) {
			...
			string.append("Dieses TemperaturVerlauf-Object enthält " + this.getCount() + " Objekte vom Typ Temperatur, "
					+ "\n" + "die durchschnittliche Temperatur ist " + Temperatur.convertKelvinToCelsius(this.calculateAverageKelvin())
					+ " und die maximale und minimale Temperatur sind: " + Temperatur.convertKelvinToCelsius(this.getMax().getKelvin()) + " / "
